class CreateFunctionCreditCardByMonth < ActiveRecord::Migration[6.0]
  def change
    execute <<-SQL
      ...
      BEGIN
        ...
        SELECT make_date(_year, _month, _credit_card.due_day) INTO _due_date;

        IF _credit_card.closing_day <= _credit_card.due_day THEN
          SELECT make_date(_year, _month, _credit_card.closing_day) INTO _closing_date;
        ELSE
          SELECT date_trunc('day', make_date(_year, _month, _credit_card.closing_day) - interval '1 month')
            INTO _closing_date;
        END IF;
